...
class _CareerChildNodesPageState extends State<CareerChildNodesPage> {
  ...
  void _navigateToDetails(String nodeId) {
    if (_isNavigating) return;
    // ✅ Use existing FetchCareerDetailsEvent instead of FetchCareerNodeDetails
    context.read<CareerSearchBloc>().add(FetchCareerDetailsEvent(nodeId));
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: BlocListener<CareerSearchBloc, CareerSearchState>(
        listener: (context, state) {
          if (state is CareerDetailsLoaded && !_isNavigating) {
            setState(() => _isNavigating = true);
            final details = state.careerDetails;
            context.push('/course-detail', extra: {
              'id': details.id,  // ✅ ADD THIS LINE
              'title': details.title,
              'description': details.description,
              'subjects': details.subjects,
              'career_options': details.careerOptions,
              'video': details.video,
              'thumbnail': details.thumbnail,
            }).then((_) {
              if (mounted) setState(() => _isNavigating = false); // ✅ ADD THIS
            });
          } else if (state is CareerDetailsError) {
                      ...
                      child: GridView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return CareerSearchResultCard(
                            ...
                            onTap: () => _navigateToDetails(node.id),
                          );
                        },
                      ),
                    ),
